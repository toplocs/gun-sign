...
Gun.on('create', function(root){
    ...
    // Wait for storage interface to be initialized by checking periodically
    var attempts = 0;
    var maxAttempts = 10;
    var checkInterval = 10; // milliseconds
    
    function initializeStore() {
        attempts++;
        
        // Check if we have a storage interface
        if(!opt.store) {
            if(attempts < maxAttempts) {
                setTimeout(initializeStore, checkInterval);
                return;
            } else {
                return;
            }
